...
function stickevent(o,ev,ptr){
	if(o.addEventListener)o.addEventListener(ev,ptr,false);
	else if(o.attachEvent)o.attachEvent('on'+ev,ptr);
	}
...
function getChartData(){
	if(getele('divLvlSlider').innerHTML==''){
		$("#divLvlSlider").slider({range:true,min:0,max:90,values:[0,90],slide:function(event,ui){
			getele('divTtlLevel').innerHTML="Level Range: "+ui.values[0]+' - '+ui.values[1];
		},change:function(event,ui){myChartData.masks.level[0]=$(this).slider('values')[0];
...
if(getele('divLoadingBar').innerHTML!=''){
	$('#divLoadingBar').progressbar('value',0);getele('divLoadingBar').style.display='block';
	}
...
stickevent(req,'progress',function(evt){
	if((typeof __jsonsize!='undefined')&&(!isNaN(parseInt(__jsonsize)))){
		if(getele('divLoadingBar').innerHTML=='')$('#divLoadingBar').progressbar({value:0});
		$('#divLoadingBar').progressbar('value',(evt.loaded/__jsonsize)*100);
		}
if(evt.loaded>0)getele('divLoadingText').innerHTML='Loading data.. ('+Math.round(evt.loaded/1024)+'kB)';});

req.open('GET',realmsetslug+'.json',true);

	req.onreadystatechange=function(evt){var json;
...
function drawCharts(p_jsondata){
	...
	if(typeof p_jsondata != 'undefined') myChartData.jsondata = p_jsondata;
	
	else if(typeof myChartData.jsondata == 'undefined') return;
	...
	if(!isRealmset){
		for(_gender in myChartData.jsondata.characters)if((myChartData.masks.gender=='')||(myChartData.masks.gender==_gender)){
			...
			for(_class in myChartData.jsondata.characters[_gender])if((myChartData.masks.classs=='')||(myChartData.masks.classs==_class)){
				...
				for(_race in myChartData.jsondata.characters[_gender][_class])if(((myChartData.masks.race=='')||(myChartData.masks.race==_race))&&((myChartData.masks.faction=='')||(myChartData.masks.faction==factionlookup[_race]))){_faction=factionlookup[_race];
				...
				if(typeof myChartData.jsondata.characters[_gender][_class][_race][_level]=='undefined')myChartData.jsondata.characters[_gender][_class][_race][_level]=[];x=myChartData.jsondata.characters[_gender][_class][_race][_level].length;counts.gender[_gender]+=x;counts.classs[_class]+=x;counts.race[_race]+=x;counts.level[_level]+=x;counts.faction[_faction]+=x;counts.all+=x;for(y=0;y<x;y++){if(myChartData['chtList'].pointcount>=250)break;myChartData['chtList'].pointcount++;myChartData['chtList'].datatoadd.push([{v:myChartData.jsondata.characters[_gender][_class][_race][_level][y],f:'<a href="http://'+myChartData.jsondata.meta.realmset+'.battle.net/wow/en/character/'+myChartData.jsondata.meta.slug+'/'+encodeURIComponent(myChartData.jsondata.characters[_gender][_class][_race][_level][y])+'/simple">'+myChartData.jsondata.characters[_gender][_class][_race][_level][y]+'</a>'},_level,_gender,_race,_class]);
				}
				}
				}
				}
							...
							for(_timezone in myChartData.jsondata.demographics[_pvp][_rp][_region])if((myChartData.masks.timezone=='')||(myChartData.masks.timezone==_timezone)){
								...
								for(_gender in myChartData.jsondata.demographics[_pvp][_rp][_region][_timezone])if((myChartData.masks.gender=='')||(myChartData.masks.gender==_gender)){
									...
									for(_class in myChartData.jsondata.demographics[_pvp][_rp][_region][_timezone][_gender])if((myChartData.masks.classs=='')||(myChartData.masks.classs==_class)){
										...
										for(_race in myChartData.jsondata.demographics[_pvp][_rp][_region][_timezone][_gender][_class])if(((myChartData.masks.race=='')||(myChartData.masks.race==_race))&&((myChartData.masks.faction=='')||(myChartData.masks.faction==factionlookup[_race]))){_faction=factionlookup[_race];
										
											if(typeof counts.race[_race]=='undefined')counts.race[_race]=0;
											
											if(typeof counts.faction[_faction]=='undefined')counts.faction[_faction]=0;
											
											for(_level=myChartData.masks.level[1];_level>=myChartData.masks.level[0];
											...
											_level--){
												...
												x=myChartData.jsondata.demographics[_pvp][_rp][_region][_timezone][_gender][_class][_race][_level];counts.pvp[_pvp]+=x;
												
												counts.rp[_rp]+=x;counts.region[_region]+=x;
												
												counts.timezone[_timezone]+=x;counts.gender[_gender]+=x;
												
												counts.classs[_class]+=x;counts.race[_race]+=x;counts.level[_level]+=x;
												
												counts.faction[_faction]+=x;counts.all+=x;
											}
										}
									}
								}
	...
	if(isRealmset){
		...
		for(_slug in myChartData.jsondata.realms){
			...
			if(realmpass){myChartData['chtList'].pointcount++;
			
			myChartData['chtList'].datatoadd.push([{v:myChartData.jsondata.realms[_slug]['name'],f:'<a href="'+__realmset+'-'+_slug+'.html">'+myChartData.jsondata.realms[_slug]['name']+'</a>'},myChartData.jsondata.realms[_slug].stats.pvp,myChartData.jsondata.realms[_slug].stats.rp,myChartData.jsondata.realms[_slug].stats.region,myChartData.jsondata.realms[_slug].stats.timezone,myChartData.jsondata.realms[_slug].counts['Alliance'],myChartData.jsondata.realms[_slug].counts['Horde'],myChartData.jsondata.realms[_slug].counts['Alliance']/myChartData.jsondata.realms[_slug].counts['Horde'],myChartData.jsondata.realms[_slug].counts['Alliance']+myChartData.jsondata.realms[_slug].counts['Horde']+myChartData.jsondata.realms[_slug].counts['Unknown']]);
			}
		}
	}
	...
	var colors;myChartData['chtList'].data.addRows(myChartData['chtList'].datatoadd);
	...
	if(!isRealmset){
		...
		myChartData['chtGuildList'].datatoadd=[];myChartData['chtGuildList'].data=new google.visualization.DataTable();
	
		myChartData['chtGuildList'].pointcount=0;myChartData['chtGuildList'].pointcounts=[];
		...
		if((_faction=='')||(_faction==_gfaction)){
			counts.guilds+=myChartData.jsondata.guilds[_gfaction].length;myChartData['chtGuildList'].pointcounts[_gfaction]=0;
			for(y=0;y<myChartData.jsondata.guilds[_gfaction].length;y++){
				...
				myChartData['chtGuildList'].datatoadd.push([(myChartData.jsondata.guilds[_gfaction][y].guild=='')?{v:'',f:'&nbsp;'}:{v:myChartData.jsondata.guilds[_gfaction][y].guild,f:'<a href="http://'+myChartData.jsondata.meta.realmset+'.battle.net/wow/en/guild/'+myChartData.jsondata.meta.slug+'/'+encodeURIComponent(myChartData.jsondata.guilds[_gfaction][y].guild)+'/">'+myChartData.jsondata.guilds[_gfaction][y].guild+'</a>'},_gfaction,myChartData.jsondata.guilds[_gfaction][y].membercount]);
		}
		}
	...
	if(isRealmset){
		...
		if(typeof myChartData['chtByPvP'].chart=='undefined'){
			...
			google.visualization.events.addListener(myChartData['chtByPvP'].chart,'select',function(){
				...
				myChartData.masks.pvp=(myChartData.masks.pvp==newmask)?'':newmask;drawCharts();
			});
		}
		
		myChartData['chtByPvP'].chart.draw(myChartData['chtByPvP'].data,{backgroundColor:'#FFFFFF',legend:{position:'none'},chartArea:{width:'95%',height:'95%'},colors:colors,is3D:true,pieSliceText:'label'});
		...
		for(_rp in counts.rp){ta.push([_rp,counts.rp[_rp],colorset[_rp]?colorset[_rp]:'#999999']);
		}
		...
		if(typeof myChartData['chtByRP'].chart=='undefined'){
			...
			google.visualization.events.addListener(myChartData['chtByRP'].chart,'select',function(){
				...
				myChartData.masks.rp=(myChartData.masks.rp==newmask)?'':newmask;drawCharts();
			});
		}
		
		myChartData['chtByRP'].chart.draw(myChartData['chtByRP'].data,{backgroundColor:'#FFFFFF',legend:{position:'none'},chartArea:{width:'95%',height:'95%'},colors:colors,is3D:true,pieSliceText:'label'});
		...
		if(typeof myChartData['chtByRegion'].chart=='undefined'){myChartData['chtByRegion'].chart=new google.visualization.PieChart(document.getElementById('chtByRegion'));
		...
		google.visualization.events.addListener(myChartData['chtByRegion'].chart,'select',function(){
			...
			myChartData.masks.region=(myChartData.masks.region==newmask)?'':newmask;drawCharts();
		});
		}
		
		myChartData['chtByRegion'].chart.draw(myChartData['chtByRegion'].data,{backgroundColor:'#FFFFFF',legend:{position:'none'},chartArea:{width:'95%',height:'95%'},is3D:true,pieSliceText:'label'});
	...
	if(typeof myChartData['chtByTimezone'].chart=='undefined'){
		...
		google.visualization.events.addListener(myChartData['chtByTimezone'].chart,'select',function(){
			...
			myChartData.masks.timezone=(myChartData.masks.timezone==newmask)?'':newmask;drawCharts();
		});
	}
	
	myChartData['chtByTimezone'].chart.draw(myChartData['chtByTimezone'].data,{backgroundColor:'#FFFFFF',legend:{position:'none'},chartArea:{width:'95%',height:'95%'},is3D:true,pieSliceText:'label'});
	}
	...
	myChartData['chtByGender'].chart.draw(myChartData['chtByGender'].data,{backgroundColor:'#FFFFFF',legend:{position:'none'},chartArea:{width:'95%',height:'95%'},colors:colors,is3D:true,pieSliceText:'label'});
	...
	if(typeof myChartData['chtByRace'].chart=='undefined'){
		...
		google.visualization.events.addListener(myChartData['chtByRace'].chart,'select',function(){
			...
			myChartData.masks.race=(myChartData.masks.race==newmask)?'':newmask;drawCharts();
		});
	}
	
	myChartData['chtByRace'].chart.draw(myChartData['chtByRace'].data,{backgroundColor:'#FFFFFF',legend:{position:'none'},chartArea:{width:'95%',height:'95%'},colors:colors,is3D:true,pieSliceText:'label'});
	...
	ormatter.format(myChartData['chtByClass'].data,1);
	...
	if(typeof myChartData['chtByClass'].chart=='undefined'){
		...
		google.visualization.events.addListener(myChartData['chtByClass'].chart,'select',function(){
			...
			myChartData.masks.classs=(myChartData.masks.classs==newmask)?'':newmask;drawCharts();
		});
	}
	
	myChartData['chtByClass'].chart.draw(myChartData['chtByClass'].data,{backgroundColor:'#FFFFFF',legend:{position:'none'},chartArea:{width:'95%',height:'95%'},colors:colors,pieSliceTextStyle:{color:'black'},is3D:true,pieSliceText:'label'});
	...
	if(typeof myChartData['chtByFaction'].chart=='undefined'){
		...
		google.visualization.events.addListener(myChartData['chtByFaction'].chart,'select',function(){
			...
			myChartData.masks.faction=(myChartData.masks.faction==newmask)?'':newmask;drawCharts();
		});
	}
	
	myChartData['chtByFaction'].chart.draw(myChartData['chtByFaction'].data,{backgroundColor:'#FFFFFF',legend:{position:'none'},chartArea:{width:'95%',height:'95%'},colors:colors,pieSliceTextStyle:{color:'white'},is3D:true,pieSliceText:'label'});
	
	if(getele('divLoading'))getele('divLoading').style.display='none';
	
	if(getele('divAllCharts'))getele('divAllCharts').style.visibility='visible';window.setTimeout(function(){myChartData.drawing=false;
		
	if(myChartData.drawagain)drawCharts();
	else if(getele('tblCharts'))getele('tblCharts').style.cursor='auto';},250);
}
...
function buildtablesort(nm){
	...
	for(_row in fromjson.rows){
		...
		for(_c=0;_c<fromjson.rows[_row].c.length;_c++){td=tr.appendChild(document.createElement('td'));
	...
	if(getele('cht'+nm).firstChild)getele('cht'+nm).replaceChild(tbl,getele('cht'+nm).firstChild);
	
	else getele('cht'+nm).appendChild(tbl);$('#tbl'+nm).tablesorter({textExtraction:function(node){
